...
## Cross-question QA auditing

Need a fully model-driven faithfulness probe? `qa_eval/` offers a question-answer based audit that stays decoupled from the BooookScore core. The tool asks an LLM to write factual questions from the summary and the source text separately, then cross-examines each side:

- **Hallucination score** – summary-derived questions are checked against the original text. A high ratio means the summary rarely invents unsupported facts.
- **Coverage score** – source-derived questions are checked against the summary. A high ratio means the summary covers the story beats the source questions focus on.

```bash
python -m qa_eval.cli \
  --source_path data/original.json \
  --summary_path summaries/my_summary.json \
  --output_path reports/qa_eval.json \
  --api openai \
  --api_key empty \
...
Both inputs follow the familiar `{"book": "text"}` structure. See [qa_eval/README.md](qa_eval/README.md) for configuration tips, including how to reuse different models for question generation vs. judging, how to tune the retry budget when LLM responses fail to emit JSON, and how to run entirely against self-hosted OpenAI-compatible endpoints.
